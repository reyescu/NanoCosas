...
topounit=Header.ExperimentParameters(b(3)+1:c(3)-1);
...
% I add here the topography channel
ch{n_ch+1}.parameter='Topography';
ch{n_ch+1}.unit=topounit;
...
% I store the parameters here and I'll use them later
par1=squeeze(dat(:,:,1)); %sweep start
par2=squeeze(dat(:,:,2)); % sweep end
par3=squeeze(dat(:,:,3)); % x
par4=squeeze(dat(:,:,4)); % y
%topography
par5=squeeze(dat(:,:,5));
ch{n_ch+1}.s1=par5;
